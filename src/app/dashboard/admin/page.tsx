...
import type { LeaveRequest, UserProfile } from '@/types';
...
export default function AdminDashboardPage() {
  ...
  const { data: userData, isLoading: isUserDataLoading } = useDoc<UserProfile>(user, userDocRef);
  ...
  const { data: usersData, isLoading: isUsersLoading } = useCollection<UserProfile>(user, allUsersQuery);
  
  const [dashboardData, setDashboardData] = useState({
    allAttendanceData: [] as DocumentData[],
    pendingLeaveRequests: [] as LeaveRequest[],
  });
    ...
    const fetchDashboardData = async () => {
        ...
        try {
            ...
            const allAttendance = attendanceSnap.docs
                .map(d => ({ ...d.data(), id: d.id } as DocumentData))
                .filter(att => {
            ...
            const allPendingLeave = leaveSnap.docs
                .map(d => ({ ...d.data(), id: d.id } as LeaveRequest))
                .filter(req => {
  ...
  const {
    ...
    if (!usersData || !isAdmin) {
      return { totalUsers: 0, kepalaSekolahCount: 0, guruCount: 0, pegawaiCount: 0 };
    }
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <Card className="lg:col-span-2">
                <CardHeader>
                    ...
                    <CardDescription>Aktivitas kehadiran semua pengguna (guru dan staf) yang tercatat hari ini.</CardDescription>
                </CardHeader>
                <CardContent>
                ...
                <Card>
                 ...
                 <Card>
                    ...
                    <CardContent>
                        ...
                        <p className="text-xs text-muted-foreground">{kepalaSekolahCount} Kepsek, {guruCount} Guru, {pegawaiCount} Pegawai</p>
                    </CardContent>
                </Card>
